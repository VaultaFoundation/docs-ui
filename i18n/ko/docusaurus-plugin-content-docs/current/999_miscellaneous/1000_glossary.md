...
## 블록 생성 노드
활발하게 블록을 생성하는 노드를 실행하는 풀 노드.생산 노드가 부결되면 “대기” 상태인 생산 노드가 됩니다.예비 프로듀서는 블록을 생산하고 투표 지분에 비례하여 보상을 받게 됩니다.
...
## 코어
코어는 네이티브 액션, 체인 라이브러리, 노데오 데몬 등과 같은 Antelope 블록체인 네이티브 구성 요소를 가리키는 데 사용되며, 핵심은 Antelope 기반 블록체인을 인스턴스화하고 스마트 계약 (시스템 스마트 계약 포함) 배포를 통해 조정할 수 있는 Antelope 플랫폼입니다.따라서 시스템 스마트 계약은 핵심 또는 기본 블록체인 구현의 일부로 간주되지 않습니다.
...
## 넷
NET은 Antelope 기반 블록체인에 트랜잭션을 저장하는 데 필요합니다.계정에 있는 NET의 양은 바이트 단위로 측정되며, 이는 새 거래를 생성할 때 계정이 사용할 수 있는 트랜잭션 스토리지의 양을 나타냅니다.NET은 각 블록이 생성된 후 계정에서 NET 대역폭에 스테이킹한 시스템 토큰을 기반으로 다시 계산됩니다.계좌에 할당되는 금액은 모든 계좌가 NET에 스테이킹한 총 시스템 토큰에 비례합니다.NET을 RAM과 혼동하지 마십시오. NET은 저장 공간이기도 하지만 계약 상태가 아니라 트랜잭션의 크기를 측정합니다.
...
## 스탠바이 풀
블록 프로듀서로 선정되고 싶다는 의사를 표명하고 온디맨드 방식으로 작업을 수행할 수 있는 약 100개의 풀 노드 세트입니다.체인이 기존 BP를 새 BP로 교체해야 할 때마다 스탠바이 풀에서 새 BP를 가져옵니다.

## 시스템
SYS는 Antelope 기반 블록체인의 블록체인 기본 토큰 이름입니다.Antelope 오픈 소스의 모든 포크는 매우 간단한 절차를 통해 심볼 검증 규칙을 충족하는 토큰 이름으로 이름을 바꿀 수 있습니다.
...
## 거래
블록체인의 전체 또는 전무에 대한 완전한 변화.하나 이상의 액션의 조합.일반적으로 스마트 계약의 실행 결과입니다.
