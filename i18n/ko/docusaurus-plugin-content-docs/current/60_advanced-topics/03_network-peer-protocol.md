...
| | commit_block | 포크 db에 블록을 추가한 후 (블록을 생성한 사람이 자신일 때만, 다른 사람으로부터 받은 블록에는 해당되지 않음) |
...
| 모듈 | 사용법 |
| --- | --- |
| 체인_플러그인 | 수락된_거래_채널로 데이터 전달 |

<const transaction_trace_ptr&, const signed_transaction&>##### 적용_트랜잭션 (캐리 표준: :튜플)
...
2.예정된 거래가 블록체인에 푸시되는 경우
 1.트랜잭션이 성공적으로 실행됨/주관적으로 실패함/소프트 페일/하드 페일-> `accepted_transaction` 컨트롤러에서 방출됩니다.
 2. chain_plugin은 신호에 반응하여 트랜잭션_메타데이터를 수락_트랜잭션_채널로 전달합니다.
3.블록이 블록체인에 푸시되는 경우 (RPC를 통해 또는 피어에 의해 브로드캐스트됨)
 ...
 3.포크 db에 블록이 추가된 후 -> `accepted_block_header` 컨트롤러에서 방출됩니다.
...
1.돌이킬 수 없는 블록 재생 시 -> `irreversible_block` 컨트롤러에서 방출됩니다.
2.방법을 보려면 A.5를 참조하십시오. `irreversible_block` 신호가 응답됨
3.리버서블 블록을 재생할 때, 블록이 fork_db에 추가되기 전 -> `pre_accepted_block` 컨트롤러에서 방출됩니다.
4.리버서블 블록 리플레이 시, 포크 db에 블럭을 추가한 후 -> `accepted_block_header` 컨트롤러에서 방출됩니다.
...
* 푸시된 블록 (새 헤드 블록) 이 어느 브랜치에서 생성될지 해결하세요.
...
첫 번째 경우에는 추가 처리를 위해 네트워크로부터 수신되는 각 직렬화된 객체 또는 메시지를 다른 쪽 끝에서 역직렬화해야 합니다.후자의 경우, 해당 콘텐츠의 암호화 해시를 생성하려면 객체 인스턴스 내의 특정 필드를 직렬화해야 합니다.지정된 객체 유형 (액션, 트랜잭션, 블록 등) 에 대해 생성되는 대부분의 ID는 객체 인스턴스의 관련 필드에 대한 암호화 해시로 구성됩니다. 
...
데이터 메시지에는 블록 내용 또는 트랜잭션 내용이 포함됩니다.제어 메시지를 통해 노드와 피어 간에 블록과 트랜잭션을 동기화할 수 있습니다 (참조). [프로토콜 메시지](#42-protocol-messages)).이러한 동기화를 허용하려면 각 노드가 자체 블록 및 트랜잭션 상태와 피어 상태에 대한 정보를 검색할 수 있어야 합니다.
...
피어 키는 피어에 연결하려는 노드의 공개 키에 해당합니다.인증에 성공하면 수신 노드는 핸드셰이크 메시지를 다시 전송하여 연결 노드를 승인합니다. 연결 노드는 위와 같은 방식으로 유효성을 검사합니다.마지막으로 수신 노드는 피어의 헤드 블록 또는 자체 헤드 블록에 동기화가 필요한지 여부를 확인합니다.이는 헤드 블록의 상태와 연결 노드의 LIB 상태를 자체 노드와 비교하여 확인하여 수행됩니다.이러한 검사를 통해 수신 노드는 동기화가 필요한 체인을 결정합니다.
